...
> ✅ В версии v3.2 пользователи создаются прямо на сайте через Edge Function `manage-user`: UUID из `auth.users` вводить не нужно.
...
### Вариант 1 — через сайт, без Supabase Dashboard

Подходит для обычного использования. После деплоя Edge Function `manage-user` owner создаёт пользователей прямо в панели:

1. Зайдите на сайт под owner-аккаунтом.
2. Откройте **Пользователи**.
3. Нажмите **+ Создать пользователя**.
4. Укажите email, пароль, имя/ник, роль и при необходимости привязку к записи в составе.
5. Нажмите **Создать**.

Никакой UUID вручную вводить не нужно. Edge Function сама создаёт Auth-аккаунт в Supabase и профиль в `user_profiles`.

### Вариант 2 — продвинутый (Edge Function `manage-user`)
...
3. Создайте функцию:
   ...
   ```
4. Замените содержимое `supabase/functions/manage-user/index.ts` на:
...
Добавлены отделы/разделы, подсчёт принятых рапортов, группировка состава и повышений по отделам, кнопки повышения/понижения, автоматическое подтягивание ранга/должности в наказания и новый визуальный слой LiveRP.

Для существующей базы выполните в Supabase SQL Editor файл `liverp_update_v3.sql`.
